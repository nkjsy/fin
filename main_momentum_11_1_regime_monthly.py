...
TOP_N_DOWN = 10
MA_DAYS = 200
OUT_PNG = 'monthly_regime_switch.png'
...
def run_backtest():
    ...
    for date, prices_row in close_matrix.iterrows():
        ...
        if date in rebalance_dates_set:
            ...
            selected_base = sel_up.get(date, []) if ok else sel_down.get(date, [])
            selected_symbols = [s for s in selected_base if s in prices.index and prices[s] > 0]
            ...
            row = {'Datetime': date, 'Mode': 'Top5' if ok else 'Top10'}
            for idx, s in enumerate(selected_symbols, start=1):
                row[f'Rank_{idx}'] = s
    ...
    plt.title('11-1 Momentum: Top5 above MA200 / Top10 below MA200')
    plt.xlabel('Date')
    plt.ylabel('Equity ($)')
